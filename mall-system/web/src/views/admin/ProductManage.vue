      ...
      <el-card>
        <el-form ref="searchFormComponents" :model="searchForm" inline>
          ...
          <el-form-item label="审核状态" prop="reviewStatus">
            <el-select v-model="searchForm.reviewStatus" placeholder="请选择" clearable style="width: 150px">
      ...
      <el-card>
        <el-table ref="tableComponents"
          ...
          <el-table-column prop="reviewStatus" label="审核状态" width="100">
            <template #default="scope">
              <el-tag v-if="scope.row.reviewStatus === '已通过'" type="success">已通过</el-tag>
          ...
          <el-table-column fixed="right" label="操作" width="340">
            <template #default="scope">
              <el-button type="success" @click="reviewProduct(scope.row.id, '已通过')" v-if="scope.row.reviewStatus === '待审核'">通过</el-button>
              <el-button type="warning" @click="reviewProduct(scope.row.id, '已驳回')" v-if="scope.row.reviewStatus === '待审核'">驳回</el-button>
...
function reviewProduct(id, reviewStatus) {
  let msg = reviewStatus === '已通过' ? '确定通过该内容审核？' : '确定驳回该内容？'
  ElMessageBox.confirm(msg, '审核确认', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(() => {
    request.put(`/product/review/${id}?reviewStatus=${reviewStatus}`).then(res => {
      if (!res) return
      ElMessage({ message: '审核操作成功', type: 'success' })
      getPageList()
    })
  }).catch(() => {})
}
