      ...
      <el-space direction="vertical" alignment="left" style="width: 100%">
        <el-table ref="tableComponents"
          ...
          <el-table-column prop="price" label="" width="200">
            <template #default="scope">
              <span style="color: red;font-size: 16px">￥{{ Math.round(scope.row.quantity * scope.row.productPrice * 100) / 100 }} </span>
            </template>
          </el-table-column>
...
const totalPrice = computed(() => {
    const centsTotal = selectionRows.value.reduce((sum, item) => {
        // 将单价和数量都转换为整数分计算
        const priceInCents = Math.round(item.productPrice * 100)
        const quantity = item.quantity
        return sum + (priceInCents * quantity)
    }, 0)

    // 转换回元单位并保留两位小数
    return (centsTotal / 100).toFixed(2)
})
